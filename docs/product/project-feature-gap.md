...
| `POST /admin/places/{placeId}/materials`        | Создать материал для места                               | `admin` | Implemented in branch `codex/admin-material-forms`: drawer form on place detail                                      |
| `PATCH /admin/materials/{materialId}`           | Редактировать материал                                   | `admin` | Implemented in branch `codex/admin-material-forms`: drawer form with dirty diff chips                                |
...
- material create/edit drawer forms on `/places/:placeId` are implemented in branch `codex/admin-material-forms`: create uses `POST /admin/places/{placeId}/materials`, edit uses partial `PATCH /admin/materials/{materialId}`, edit drawer shows dirty diff chips and both drawers guard dirty close;
...
- merge material create/edit drawer forms from branch `codex/admin-material-forms`;
...
- Status: first read-only materials panel is done in admin `main` via merged admin PR [#8](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/8). Backend bounded materials read is done via merged backend PR [#43](https://github.com/DenisChernykh/amazing-ekb-hub-backend/pull/43), closing backend issue [#27](https://github.com/DenisChernykh/amazing-ekb-hub-backend/issues/27) and [#28](https://github.com/DenisChernykh/amazing-ekb-hub-backend/issues/28). Admin PR [#10](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/10) is merged to `main`: generated client is synced and the panel uses `GET /admin/places/{placeId}/materials`; tracked admin issue [#7](https://github.com/DenisChernykh/amazing-ekb-hub-admin/issues/7) is closed. Create/update material forms are implemented in branch `codex/admin-material-forms`.

- Добавить materials tab на странице места.
- Реализовать create/update material forms. Implemented in branch `codex/admin-material-forms`: AntD Drawer, dirty close guard, edit diff chips, partial PATCH payload, materials/detail cache invalidation.
