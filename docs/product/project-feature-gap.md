...
| Admin bulk moderation            | Есть одиночный `PATCH /admin/places/{placeId}/status`; backend bulk endpoint не нужен для первого frontend-heavy среза                                                                                                                                                                                                                 | Не требуется                                                                                                                                | Backlog: client-side bulk status workflow over existing single-place endpoint; tracked in [admin issue #15](https://github.com/DenisChernykh/amazing-ekb-hub-admin/issues/15) and [Notion roadmap](https://www.notion.so/356756c470b381a2837cf46bfd122dc6)                                                                                                   | Нужны Redux Toolkit workflow state, selection across pages, progress, partial errors, retry failed, undo |
...
- Status: backlog; tracked in admin issue [#15](https://github.com/DenisChernykh/amazing-ekb-hub-admin/issues/15) and [Notion roadmap](https://www.notion.so/356756c470b381a2837cf46bfd122dc6). This is an intentional Learning / Frontend architecture slice for Redux Toolkit.
...
- State ownership: Redux Toolkit stores selection, operation queue, per-item progress, retry/undo metadata; React Query remains the source of truth for server data.
- Non-goals: no bulk delete, no material bulk actions, no backend bulk endpoint in this slice.
- Acceptance: selection across pages, operation progress, partial failures, retry failed items, undo through reverse status updates, and no regression in the existing single-place status flow.
