...
- Добавить create form для `title`, `summary`, `tags`, `category`, `popularityWeight`. Done: route `/places/new`, AntD Form, category options, create mutation bridge, list invalidation, success/error feedback. Optional cover photo on create is done via merged admin PR [#34](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/34): create runs first, upload uses the created place id, success navigates to detail, and upload failure keeps a partial-success retry path.
