...
| `PATCH /admin/places/{placeId}`                 | Редактировать место                                      | `admin` | Есть edit form в branch         |
...
- route `/places/:placeId/edit` с формой редактирования места через `PATCH /admin/places/{placeId}` в branch `codex/admin-edit-place`;
...
| Admin update place               | Есть                                                     | Не требуется                                          | In branch: route `/places/:placeId/edit`, dirty guard, diff PATCH | Нужен review/merge и runtime smoke с реальным backend                       |
...
- Status: create place slice done in admin `main`, commits `bbc4284` and `2006c9f`; status panel done in admin `main` via merged admin PR [#3](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/3); edit slice implemented in branch `codex/admin-edit-place`.
...
- Добавить edit form для `title`, `summary`, `tags`, `category`, `popularityWeight`. In branch: route `/places/:placeId/edit`, partial diff payload, reset to server values, dirty navigation blocker.
