...
| `PATCH /admin/places/{placeId}`                 | Редактировать место                                      | `admin` | Edit form in admin PR #4        |
...
- route `/places/:placeId/edit` с формой редактирования места через `PATCH /admin/places/{placeId}` в draft admin PR [#4](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/4);
...
| Admin update place               | Есть                                                     | Не требуется                                          | In draft admin PR [#4](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/4): route `/places/:placeId/edit`, dirty guard, diff PATCH | Нужен review/merge и runtime smoke с реальным backend                       |
...
- Status: create place slice done in admin `main`, commits `bbc4284` and `2006c9f`; status panel done in admin `main` via merged admin PR [#3](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/3); edit slice is in draft admin PR [#4](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/4).
...
- Добавить edit form для `title`, `summary`, `tags`, `category`, `popularityWeight`. In draft admin PR [#4](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/4): route `/places/:placeId/edit`, partial diff payload, reset to server values, dirty navigation blocker.
