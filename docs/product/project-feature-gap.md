...
| `GET /admin/places/{placeId}/materials`         | Bounded список материалов места, включая `hidden` places | `admin` | Sync implemented in draft admin PR [#10](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/10) |
...
4. Admin read endpoint для материалов hidden places готов.
   ...
   - Draft admin PR [#10](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/10) переключает read-only panel на bounded admin endpoint.
