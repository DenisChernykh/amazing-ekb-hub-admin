...
| `GET /admin/places/{placeId}/materials`         | Bounded список материалов места, включая `hidden` places | `admin` | Есть sync в branch `codex/admin-materials-contract-sync` |
...
- read-only materials panel на `/places/:placeId` в admin `main` через merged admin PR [#8](https://github.com/DenisChernykh/amazing-ekb-hub-admin/pull/8); branch `codex/admin-materials-contract-sync` переключает panel на bounded `GET /admin/places/{placeId}/materials`, включая `hidden` places;
...
| Admin create place               | Есть; `summary`/`tags` optional in backend PR [#41](https://github.com/DenisChernykh/amazing-ekb-hub-backend/pull/41), closing [backend issue #26](https://github.com/DenisChernykh/amazing-ekb-hub-backend/issues/26)                                                                                 | Не требуется                                                                                                                                | Есть: route `/places/new`, AntD create form, validation feedback; admin issue [#5](https://github.com/DenisChernykh/amazing-ekb-hub-admin/issues/5) remains for UI optional fields  | Нужен runtime smoke с реальным backend                   |
...
4. Admin read endpoint для материалов hidden places готов.
   ...
   - Admin branch `codex/admin-materials-contract-sync` переключает read-only panel на bounded admin endpoint.
